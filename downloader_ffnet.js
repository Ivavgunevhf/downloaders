...
function getStory(url, id) {
	fetch(url).then(function(response) {
		...
		if (doc.querySelector('#profile_top button') != null) {
			// Check that story still exists
			var numberOfChapters = doc.querySelectorAll('#chap_select option').length / 2;
			// divison by 2; bc ffnet has two chapter selctors.

			if (numberOfChapters == 0) {
				numberOfChapters = 1;
			}

			doc.querySelector('#profile_top button').remove();
			// remove this then get the text.
			
			// This section is to change the date to be MM/DD/YY no matter when it was updated.
			var bottom = doc.querySelector('#profile_top span:last-child').innerText;
			var bottomHTML = doc.querySelector('#profile_top span:last-child').innerHTML;
			var count = doc.querySelectorAll('#profile_top span [data-xutime]').length;
			var complete = doc.querySelector('#profile_top').innerHTML.indexOf('Complete')>=0;
			doc.querySelector('#profile_top span:last-child').remove();

			// Handle the date issues 
			if (count==2) {
				var time1 = bottomHTML.split('data-xutime=\"')[1].split('"')[0];
				var time2 = bottomHTML.split('data-xutime=\"')[2].split('"')[0];
				var sections = bottom.split('Updated: ');
				var part1 = sections[0];
				var part2 = sections[1].split('Status: ')[1]
				
				bottom = part1 + 'Updated: ' + convertTimeStamp(time1) + ' - Published: ' + convertTimeStamp(time2);
				
				if (complete==true){
					bottom += ' - Status: ' + part2;
				}
			} else  {
				var time = bottomHTML.split('data-xutime=\"')[1].split('"')[0];
				var sections = bottom.split('Published: ');
				var part1 = sections[0];
				var part2 = sections[1].split(' - ')[1];
				bottom = part1 + 'Published: ' + convertTimeStamp(time) +' - '+ part2;
			}
			// Have it ready for the recombine with the rest of the info. 
			...
			for (var i = 1; i < numberOfChapters + 1; i++) {
				// BC this doesnt play nice make temp fields to hold the text before the great combining.
				//console.log(url+'/'+i);
...
function combiner(id) {
	...
	$('#' + id + ' textarea').each(function() {
		if ($(this).is('#combined_' + id) != true) {
			// skip the combined form or you get it twice.
...
function checkFile(id, ch) {
	if ((($('#combined_' + id).val().match(/storytext/g) || []).length - 1) != ch) {
		// Minus one bc of the css. BC of the way i wrote the check things might break.
		return false;
	} else {
		return true;
	}
...
function convertTimeStamp(unix_timestamp){
	// Month/Day/Year
	x = new Date(unix_timestamp*1000);
	month = x.getMonth() + 1; // Jan = 0
	return month+'/'+x.getDate()+'/'+x.getFullYear();
}
